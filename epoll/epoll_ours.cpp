...
int	main (int argc, char *argv[])
{
	...
	while (1)
	{
		...
		for (i = 0; i < n; i++)		{		  if ((events[i].events & EPOLLERR) ||
				(events[i].events & EPOLLHUP) ||
				(!(events[i].events & EPOLLIN)))		    {
				/* An error has occured on this fd, or the socket is not
				   ready for reading (why were we notified then?) */		      fprintf (stderr, "epoll error\n");		      close (events[i].data.fd);		      continue;		    }		  else if (sfd == events[i].data.fd)		    {
				...
				while (1)
				{
					...
					if (infd == -1)
					{
						if ((errno == EAGAIN) ||
							(errno == EWOULDBLOCK))
						{
							std::cout << "Si 2 fois pas bon" << std::endl;
